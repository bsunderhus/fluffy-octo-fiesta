...
const CLASS_NAME_REGEX = new RegExp(
  `([^\\s]*${fluentProviderClassNames.root}\\w+)`
);

export const PortalCompatProvider: React.FC<{ children?: React.ReactNode }> = (
  props
) => {
    ...
    (element) => {
      ...
      if (cssVariablesClassName) {
        ...
        if (element.ownerDocument.defaultView) {
          disposeFocusVisiblePolyfill = applyFocusVisiblePolyfill(
            element,
  ...
  if (process.env.NODE_ENV !== "production") {
    ...
    React.useEffect(() => {
      if (themeClassName === "") {
        ...
        console.warn(`PortalCompatProvider: "useThemeClassName()" hook returned an empty string
=============================================
