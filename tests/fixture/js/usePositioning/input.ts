...
export function usePositioning(
  options: PositioningProps & PositioningOptions
): UsePositioningReturn {
    ...
    () => ({
      ...
      setTarget: (target: TargetElement) => {
        if (options.target && process.env.NODE_ENV !== "production") {
          ...
          console.warn(
            "Imperative setTarget should not be used at the same time as target option"
          );
          // eslint-disable-next-line no-console
      ...
      if (containerRef.current) {
        ...
        while (treeWalker.nextNode()) {
          ...
          console.warn(/** @swc strip-indentation */ `
            ...
            We suggest to use the "trapFocus" prop on Fluent components or a catch "ref" and then use "ref.current.focus" in React.useEffect(): https://reactjs.org/docs/refs-and-the-dom.html#adding-a-ref-to-a-dom-element
          `);
        }
      }
  ...
  return React.useCallback(
    (container: HTMLElement | null, arrow: HTMLElement | null) => {
      ...
      const middleware = [
        ...
        autoSize &&
          maxSizeMiddleware(autoSize, { container, overflowBoundary }),
