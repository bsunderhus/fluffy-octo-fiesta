...
export const useFluentProviderThemeStyleTag = (
  ...
  const cssVarsAsString = React.useMemo(() => {
    return theme
      ? (Object.keys(theme) as (keyof typeof theme)[]).reduce(
          (cssVarRule, cssVar) => {
    ...
    React.useMemo(() => {
      ...
      if (targetDocument) {
        ...
        const providerElements =
          targetDocument.querySelectorAll(providerSelector);
        ...
        const isSSR =
          targetDocument.querySelector(
            `${providerSelector} > style[id="${styleTagId}"]`
          ) !== null;
        const elementsCount = isSSR ? 1 : 0;

        if (providerElements.length > elementsCount) {
          // eslint-disable-next-line no-console
          console.error(`@fluentui/react-provider: There are conflicting ids in your DOM. Please make sure that you configured your application properly. 
