...
export type SlotOptions<Props extends UnknownSlotProps> = {
		elementType: React.ComponentType<Props> | (Props extends AsIntrinsicElement<infer As> ? As : keyof JSX.IntrinsicElements);
 ...
 */ export function always<Props extends UnknownSlotProps>(value: Props | SlotShorthandValue | undefined, options: SlotOptions<Props>): SlotComponentType<Props> {
 ...
 */ export function optional<Props extends UnknownSlotProps>(value: Props | SlotShorthandValue | undefined | null, options: {
 ...
 */ export function resolveShorthand<Props extends UnknownSlotProps | null | undefined>(value: Props | SlotShorthandValue): Props {
		...
		if (value && typeof value !== "object" && process.env.NODE_ENV !== "production") {
				...
				console.error(`[slot.resolveShorthand]: A slot got invalid value "${value}" (${typeof value}).
